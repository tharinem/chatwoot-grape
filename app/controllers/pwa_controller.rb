# Inherits from ActionController::Base to skip authentication middleware.
# Serves the PWA manifest with the configured installation branding.
class PwaController < ActionController::Base # rubocop:disable Rails/ApplicationController
  def manifest
    global_config = GlobalConfig.get('INSTALLATION_NAME', 'LOGO_THUMBNAIL')
    installation_name = global_config['INSTALLATION_NAME'].presence || 'Chatwoot'

    render json: {
      name: installation_name,
      short_name: installation_name,
      icons: [
        { src: '/android-icon-36x36.png', sizes: '36x36', type: 'image/png', density: '0.75' },
        { src: '/android-icon-48x48.png', sizes: '48x48', type: 'image/png', density: '1.0' },
        { src: '/android-icon-72x72.png', sizes: '72x72', type: 'image/png', density: '1.5' },
        { src: '/android-icon-96x96.png', sizes: '96x96', type: 'image/png', density: '2.0' },
        { src: '/android-icon-144x144.png', sizes: '144x144', type: 'image/png', density: '3.0' },
        { src: '/android-icon-192x192.png', sizes: '192x192', type: 'image/png', density: '4.0' }
      ],
      start_url: '/',
      display: 'standalone',
      background_color: '#1f93ff',
      theme_color: '#1f93ff'
    }
  end
end
